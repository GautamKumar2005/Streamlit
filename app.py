...
COCO_MODEL_DIR = r"C:\Users\gauta\.project_ml\Streamlit-ANPR\License-Plate-Detection-with-YoloV8-and-EasyOCR\models\yolov8n.pt"
...
def read_license_plate(license_plate_crop, img):
    ...
    for result in detections:
        ...
        if len(text) >= 9 and length * height / rectangle_size > 0.17:
            text = text.upper()
            scores += score
            plate.append(text)
...
def write_csv(results, file_path):
    ...
    df = pd.DataFrame(data, columns=[
        ...
        'License Plate BBox Score', 'License Plate Text Score'
    ])
...
def model_prediction(img):
    ...
    if len(license_detections.boxes.cls.tolist()) != 0:
        ...
        for license_plate in license_detections.boxes.data.tolist():
            ...
            if license_plate_text and license_plate_text_score and len(license_plate_text) >= 9:
...
with st.container() as header:
    ...
    col0.image(r"C:\Users\gauta\.project_ml\Streamlit-ANPR\background.png", width=500)

    _, col4, _ = st.columns([0.1, 1, 0.2])
    col4.subheader("Computer Vision Detection with YoloV8 🧪")

    _, col, _ = st.columns([0.3, 1, 0.1])
    col.image(r"C:\Users\gauta\.project_ml\Streamlit-ANPR\background.png")

    _, col5, _ = st.columns([0.05, 1, 0.1])
    st.write("The models detect the car and the license plate in a given image, then extract the info using EasyOCR, and crop and save the license plate as an image, with a CSV file containing all the data.")
...
def improve_to_detect_blurry_video_more_accurately(video_path, csv_output_path):
    cap = cv2.VideoCapture(video_path)
    frame_rate = int(cap.get(cv2.CAP_PROP_FPS))
    frame_count = 0
